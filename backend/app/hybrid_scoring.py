...
def cleanup_temp_segments(audio_path: str):
    """
    Removes all temporary word-segment files generated by slice_audio_segment().
    Canonical TTS files remain untouched.
    """
    # Example audio_path: "uploads/audio123.wav" → "uploads/audio123"
    base = str(Path(audio_path).with_suffix(""))

    # Look for all pattern matches
    pattern = base + "_seg_*.wav"
    for filepath in glob.glob(pattern):
        try:
            os.remove(filepath)
        except Exception as e:
            print(f"[WARN] Failed to delete temp segment {filepath}: {e}")
...
def compute_per_word_scores(
    ...
    cleanup_temp_segments(audio_path)
